...
import DemoPersonalizacao from "./DemoPersonalizacao";
  ...
  return (
    <div className="pb-4">
      <div className="bg-gradient-to-br from-orange-700 to-red-800 px-4 py-5">
        <p className="text-orange-200 text-xs mb-1">🕐 Entrega: 30–45 min · Taxa: R$ 5,00</p>
        <p className="text-white text-sm font-semibold">Peça agora e receba em casa!</p>
      </div>
      ...
      <div className="px-4 space-y-3">
        ...
        {filtrados.map(produto => (
          <div key={produto.id} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 flex gap-3 items-start">
            <span className="text-4xl shrink-0">{produto.emoji}</span>
            <div className="flex-1">
              <p className="text-zinc-100 font-bold text-sm">{produto.name}</p>
              <p className="text-zinc-500 text-xs mt-0.5 leading-relaxed">{produto.description}</p>
              <p className="text-orange-400 font-black text-base mt-2">R$ {produto.price.toFixed(2).replace(".", ",")}</p>
            </div>
      ...
      {produtoPersonalizando && (
        <DemoPersonalizacao produto={produtoPersonalizando} onClose={() => setProdutoPersonalizando(null)} />
      )}
    </div>
  );
}